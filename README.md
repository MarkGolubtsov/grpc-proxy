...
Содержит готовую конфигурацию **Nginx reverse proxy** для
проксирования gRPC-Web запросов к gRPC бэкенду (на порту 8080).

### Цель

Обойти CORS ограничения при работе с gRPC-Web, позволив клиенту отправлять
запросы через единую точку входа, которая проксирует их на сервер.

### Возможности

- **HTTP/2 поддержка** - полная поддержка HTTP/2 протокола
- **gRPC-Web совместимость** - правильная обработка gRPC-Web запросов и
  ответов
- **CORS headers** - автоматическая обработка CORS preflight запросов
- **Trailing headers** - сохранение trailing headers в HTTP/2
- **Переменные окружения** - гибкая конфигурация через `.env`
